...
`damage_type_icon` в этих редакторах не настраивается: его позиция берётся из старых frame rules, где эта иконка уже стоит правильно.

## Пропорциональное изменение размера

Чекбокс `Lock proportions` включён по умолчанию. Если он включён, изменение `Width` автоматически пересчитает `Height`, а изменение `Height` автоматически пересчитает `Width` по текущим пропорциям выбранного слоя.

Если нужно растянуть слой неравномерно, временно выключи `Lock proportions`.
